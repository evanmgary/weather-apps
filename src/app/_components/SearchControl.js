...
export default function SearchControl(props){
    ...
    async function recentHandler(){
        ...
        const res = await fetch("http://localhost:5000/api/" + station.trim())
    ...
    async function historicHandler(){
        ...
        const res = await fetch(`http://localhost:5000/api/${station.trim()}/${year}`)
        const resJson = await res.json()
        if ("error" in resJson) {
            props.setGraph1Desc("Invalid station and/or year.")
            return
        ...
        props.setGraphDesc1(`Displaying station ${station} for year ${year}`)
        props.setData(resJson)
    }
